...
function getFirebaseCredential(adminInstance) {
    const serviceAccount = node_utils_1.getServiceAccount();
...
function initializeFirebase(adminInstance, overrideConfig) {
    ...
    try {
        ...
        /* eslint-disable no-console */
        console.log(`cypress-firebase: Initialized app with database url "${fbConfig.databaseURL}"`);
        /* eslint-enable no-console */
        return fbInstance;
    }
    catch (err) {
        /* eslint-disable no-console */
        console.error('cypress-firebase: Error initializing firebase-admin instance:', err.message);
        /* eslint-enable no-console */
        throw err;
    }
}
