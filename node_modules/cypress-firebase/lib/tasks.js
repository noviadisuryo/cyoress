...
async function callFirestore(adminInstance, action, actionPath, options, data) {
    ...
    try {
        if (action === 'get') {
            const snap = await firebase_utils_1.slashPathToFirestoreRef(adminInstance.firestore(), actionPath, options).get();
        ...
        if (action === 'delete') {
            ...
            const deletePromise = firebase_utils_1.isDocPath(actionPath)
                ? firebase_utils_1.slashPathToFirestoreRef(adminInstance.firestore(), actionPath, options).delete()
                : // TODO: Here the ref should be passed along instead so we can accept options
                    firebase_utils_1.deleteCollection(adminInstance.firestore(), actionPath, options === null || options === void 0 ? void 0 : options.batchSize);
            await deletePromise;
        ...
        // "update" action
        return firebase_utils_1.slashPathToFirestoreRef(adminInstance.firestore(), actionPath, options)[action](dataToSet);
    }
    catch (err) {
        /* eslint-disable no-console */
 ...
 * @returns Promise which resolves with a custom Firebase Auth token
 */
function getAuthUser(adminInstance, uid) {
